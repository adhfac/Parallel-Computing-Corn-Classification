...
logger.info("="*70)
logger.info("PARALLEL BATCH BENCHMARK STARTED")
logger.info("="*70)
...
for idx, ds in enumerate(datasets,start=1):
    ...
    logger.info(
        ...
        f"Starting PARALLEL BATCH inference..."
    )

    logger.info(
        f"[DATASET {ds}] Multiprocessing workers ACTIVE"
    )

    logger.info(
        f"[DATASET {ds}] CPU intensive workload STARTED"
    )
    ...
    logger.info(
        f"[DATASET {ds}] Parallel inference FINISHED"
    )
...
logger.info("")
logger.info("="*70)
logger.info("PARALLEL BATCH BENCHMARK COMPLETED")
logger.info("="*70)